...
func _test_river_x(t, w) -> void:
	...
	t.eq(MATH.river_x(5.0), MATH.river_x(5.0), "_river_x deterministico per z=5.0")
...
func _test_is_river(t, w) -> void:
	...
	t.ok(w.is_river(Vector3(MATH.river_x(0.0), 0.0, 0.0)),
			"is_river vero al centro del letto (z=0)")

	# Falso se lontano in x (|dx| = 5 >= 2.9).
	t.ok(not w.is_river(Vector3(MATH.river_x(0.0) + 5.0, 0.0, 0.0)),
			"is_river falso lontano dal centro in x")

	# Falso oltre il limite z superiore (z = 60 non < 56).
	t.ok(not w.is_river(Vector3(MATH.river_x(60.0), 0.0, 60.0)),
...
func _test_cliff_x(t, w) -> void:
	...
	t.almost(MATH.cliff_x(-4.0), MATH.river_x(-4.0) + 2.9,
			"_cliff_x alla cascata: solo river_x + 2.9")

	# Lontano dalla cascata (|z - (-4)| = 14 >= 9) lo smoothstep satura a 1.
	t.almost(MATH.cliff_x(10.0), MATH.river_x(10.0) + 2.9 + 9.0,
	...
	for z: float in [-56.0, -20.0, -4.0, 0.0, 3.0, 22.0, 56.0]:
		var base = MATH.river_x(z) + 2.9
		var c = MATH.cliff_x(z)
		t.ok(c >= base - 0.0001 and c <= base + 9.0 + 0.0001,
				"_cliff_x nei limiti dello smoothstep per z=%s (c=%s)" % [z, c])
		t.ok(c > MATH.river_x(z), "_cliff_x a est del centro fiume per z=%s" % z)

	# Determinismo.
	t.eq(MATH.cliff_x(3.0), MATH.cliff_x(3.0), "_cliff_x deterministico per z=3.0")
...
func _test_catmull(t, w) -> void:
	...
	t.ok(MATH.catmull(a, b, c, d, 0.0).is_equal_approx(b),
			"_catmull(t=0) == p1")

	# A t=1 restituisce esattamente p2.
	t.ok(MATH.catmull(a, b, c, d, 1.0).is_equal_approx(c),
			"_catmull(t=1) == p2")

	# Determinismo: stessi input danno lo stesso Vector3.
	t.ok(MATH.catmull(a, b, c, d, 0.37).is_equal_approx(MATH.catmull(a, b, c, d, 0.37)),
...
func _test_tuft_hash(t, w) -> void:
	...
	for coppia in [[0, 0], [3, 7], [-5, 2], [123, -456], [-999, -999]]:
		var v = MATH.tuft_hash(coppia[0], coppia[1])
		t.ok(v >= 0.0 and v <= 1.0,
				"_tuft_hash in [0,1] per (%s,%s) -> %s" % [coppia[0], coppia[1], v])

	# Determinismo.
	t.eq(MATH.tuft_hash(3, 7), MATH.tuft_hash(3, 7), "_tuft_hash deterministico (3,7)")
	t.eq(MATH.tuft_hash(-5, 2), MATH.tuft_hash(-5, 2), "_tuft_hash deterministico (-5,2)")
...
func _test_tuft_vnoise(t, w) -> void:
	...
	for p in [[0.0, 0.0], [2.3, -4.1], [-7.6, 5.5], [12.25, 12.25], [-0.5, -0.5]]:
		var v = MATH.tuft_vnoise(p[0], p[1])
		t.ok(v >= -0.0001 and v <= 1.0001,
				"_tuft_vnoise in [0,1] per (%s,%s) -> %s" % [p[0], p[1], v])

	# Determinismo.
	t.almost(MATH.tuft_vnoise(2.3, -4.1), MATH.tuft_vnoise(2.3, -4.1),
...
func _test_tuft_field(t, w) -> void:
	...
	for p in [[0.0, 0.0], [1.0, 1.0], [-8.3, 6.4], [15.0, -18.0], [-3.3, -3.3]]:
		var v = MATH.tuft_field(p[0], p[1])
		t.ok(v >= -0.0001 and v <= 1.0001,
				"_tuft_field in [0,1] per (%s,%s) -> %s" % [p[0], p[1], v])

	# Determinismo.
	t.almost(MATH.tuft_field(1.0, 1.0), MATH.tuft_field(1.0, 1.0),
