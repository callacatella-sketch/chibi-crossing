extends SceneTree
## IL PROVINO DI UN PEZZO QUALUNQUE del catalogo: sei viste (fronte -Z,
## tre quarti, profilo, tre quarti retro, retro, alto) nello stesso
## studio del catalogo. Serve a GUARDARE mentre si modella: la suite
...
const VISTE := [
	...
	{"file": "4-tre-quarti-retro.jpg", "az": 2.356194, "ele": 0.42},
	{"file": "5-retro.jpg", "az": 3.141593, "ele": 0.42},
...
func _go() -> void:
	await process_frame
	_studio()
	var nome := OS.get_environment("CHIBI_PEZZO")
	...
	for v in CAT.items():
		if str(v["name"]) == nome:
			...
			break
	if voce.is_empty():
		push_error("Pezzo non trovato: '" + nome + "' (CHIBI_PEZZO)")
		quit(1)
		return
