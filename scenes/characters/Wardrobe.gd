...
func _unhandled_input(event: InputEvent) -> void:
	...
	# indossa con E (1-9 restano scorciatoie per i primi capi)
	if event.is_action_pressed("ui_down") or event.is_action_pressed("ui_up"):
		var passo := 1 if event.is_action_pressed("ui_down") else -1
		_sel = posmod(_sel + passo, ORDER.size())
		if _sfx:
			_sfx.ui_select()
		_refresh_panel()
		get_viewport().set_input_as_handled()
	elif event.is_action_pressed("interact"):
		_toggle_wear(ORDER[_sel])
		get_viewport().set_input_as_handled()
	elif event is InputEventKey and event.pressed and not event.is_echo():
		var idx: int = (event as InputEventKey).keycode - KEY_1
		if idx >= 0 and idx < mini(9, ORDER.size()):
			_sel = idx
...
func _refresh_panel() -> void:
	for c in _rows.get_children():
		c.queue_free()
	_sel = clampi(_sel, 0, ORDER.size() - 1)
	for i in ORDER.size():
