...
# preload espliciti: non dipendono dalla cache globale delle class_name
...
func _ready():
	...
	if OS.get_environment("CHIBI_SHOT") != "":
		_debug_screenshots(OS.get_environment("CHIBI_SHOT"))
...
# ---------------------------------------------------------------- debug CLI
# Modalità verifica: CHIBI_SHOT=<cartella> salva quattro screenshot e esce.
...
func _debug_screenshots(dir: String):
	...
	($Cooking as Node).held_dish = {"name": "vellutata di zucca", "art": "la", "warm": true}
...
func _make_save() -> void:
	...
	($Cooking as Node).pantry = {"carota": 3, "fungo": 2, "bacca": 2}
	($DayNight as Node3D).day = 3

	# casa del giocatore: il letto grande è già "casa"
	($Home as Node).home_cell = Vector2i(1, -1)

	# due vicini già insediati nelle casette
	var vis: Node = $Visitors
